/* Rotation Break Tracker enhancements v7: roster order, attendance, coverage */
(()=>{
  ...
  renderManage=function(){const roster=activeTeam(),removed=team.filter(m=>m.active===false);$('#memberList').innerHTML=roster.map((m,i)=>`<div class="manage-row"><div><span class="order-badge">${i+1}</span><b>${escapeHtml(m.name)}</b></div><div class="muted small" style="margin-top:5px">Job: ${escapeHtml(m.job||'Not assigned')} · Today: ${escapeHtml(getAttendance(m.id))}</div><div class="toolbar" style="margin-top:8px"><button class="secondary" onclick="moveMember('${m.id}',-1)" ${i===0?'disabled':''}>↑ Up</button><button class="secondary" onclick="moveMember('${m.id}',1)" ${i===roster.length-1?'disabled':''}>↓ Down</button><button class="secondary" onclick="changeJob('${m.id}')">Change Job</button><button class="danger" onclick="removeMember('${m.id}')">Delete</button></div></div>`).join('')+(removed.length?`<div class="muted small" style="margin:14px 0 4px">REMOVED TEAM MEMBERS</div>${removed.map(m=>`<div class="manage-row removed"><b>${escapeHtml(m.name)}</b><div class="muted small">History retained</div><div class="toolbar" style="margin-top:8px"><button class="secondary" onclick="restoreMember('${m.id}')">Restore</button></div></div>`).join('')}`:'');};
  ...
  exportBackup=function(){downloadBlob(JSON.stringify({version:7,team,breaks,activeBreaks,repairs,attendance},null,2),'rotation-break-backup.json','application/json');};
